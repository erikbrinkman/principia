...
function arrayToPoints(array) {
    const pointArray = new Array(array.length / 2).fill(null).map(_ => [null, null]);
...
function placeLabels(bbox, boxes, curvez, options) {
    ...
    function loss(array, grad) {
        ...
        // dist between labels
        const distBetween = utils.mean([].concat(...grouped.map((pi, i) => grouped.slice(i + 1).map(pj => points.dist.point(pi, pj)))));
        // dist to other lines
        ...
        const boundaryPenalty = utils.sum(grouped.map((pi, i) => Math.pow(Math.max(...rects.to.points(pi.concat(boxes[i]))
