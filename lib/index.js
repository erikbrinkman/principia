    ...
    plot(svgElement) {
        ...
        if (this._lines.some(line => line.label().length > 0)) {
            if (this._sideLabels) {
                ...
                const lines = this._lines.map(line => line.data().map(([xi, yi]) => [x(xi), y(yi)]));
                const res = backend.space2([0, 0, this._width, this._height], boxes, lines, { lineBuffer: this._labelBuffer });
                labels.forEach((label, i) => {
                    const [x, y] = res[i];
