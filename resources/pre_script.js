...
async function alignYAxisLabel(spacing) {
  // eslint-disable-line no-unused-vars
  const label = document.querySelector('.princ--yaxis .princ--axis-label');
  label.style.transform = 'none';
  await render();
  const lbb = bbox(label);
  const tbb = Array.from(
    document.querySelectorAll('.princ--yaxis .princ--tick-label'),
  )
    .map(bbox)
...
async function alignXAxisLabel(spacing, shift) {
  // eslint-disable-line no-unused-vars
  const label = document.querySelector('.princ--xaxis .princ--axis-label');
  label.style.transform = 'none';
  await render();
  const lbb = bbox(label);
  const { x: tx, width: twidth } = bbox(
    document.querySelector('.princ--xaxis .princ--tick-line'),
  );
  const center = tx + twidth / 2;
  const tbbs = Array.from(
    document.querySelectorAll('.princ--xaxis .princ--tick-label'),
  ).map(bbox);
  const newy =
    shift &&
    tbbs.every(
...
/** Space apart y ticks */
async function spaceApartYAxisTickLabels(spacing) {
  // eslint-disable-line no-unused-vars
...
/** Space apart x ticks */
async function spaceApartXAxisTickLabels(spacing) {
  // eslint-disable-line no-unused-vars
  await spaceApartX(
    document.querySelectorAll('.princ--xaxis .princ--tick-label'),
    spacing,
...
async function alignComparisonZNames(spacing) {
  // eslint-disable-line no-unused-vars
...
async function alignComparisonZNums(spacing) {
  // eslint-disable-line no-unused-vars
...
async function spaceApartEvolutionLabels(spacing) {
  // eslint-disable-line no-unused-vars
  const { y } = bbox(document.querySelector('.princ--xaxis .princ--tick-line'));
  await spaceApartY(
