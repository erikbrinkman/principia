...
it('works sequentially', () => {
  ...
  cp.execFileSync(cmd, [
    ...
    evolutionPdf,
    '-o',
    'test/evolution.ex.append.png',
  ]);
});

it('performs all actions in a pipe', () => {
  execPipeSync(
    `< ${evolution} ${cmd} plot | ${cmd} append ${evolutionCss} | ${cmd} html | ${cmd} pdf | ${cmd} png > test/evolution.ex.pipe.png`,
  );
});

it('adjusts x axis all the way up', () => {
  execPipeSync(
    `< ${evolution} ${cmd} plot | ${cmd} append ${evolutionCss} | ${cmd} html --xaxis-label 0 | ${cmd} pdf > test/evolution.ex.xaxis.pdf`,
  );
});
...
it('adjusts evolution appropriately', () => {
  ...
  execPipeSync(
    `${cmd} plot | ${cmd} append ${evolutionCss} | ${cmd} html --label 0 --xaxis-ticks 2 --xaxis-label 0 --yaxis-ticks 0 --yaxis-label 0 | ${cmd} pdf > test/evolution.ex.adjust.pdf`,
...
it('adjusts comparisonz appropriately', () => {
  execPipeSync(
    `< ${comparisonz} ${cmd} plot | ${cmd} html --name 3 --num 3 | ${cmd} pdf > test/comparisonz.ex.adjust.pdf`,
  );
});

it('handles long svg renders', () => {
  execPipeSync(
    `< test/long_render.json ${cmd} plot | ${cmd} append <(echo '.princ--point { display: none; }') | ${cmd} html --label 0 | ${cmd} pdf > test/long_render.pdf`,
  );
});
